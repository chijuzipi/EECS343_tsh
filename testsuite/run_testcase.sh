...
if [[ ! "$#" -eq 2 ]]; then
	echo -e "usage: $0 targz_file shell_arch";
	exit 1;
fi;
...
echo "Using reference shell ${SHELL_ARCH}"
...
for tc in ${BASIC_TESTS}; do
	...
	if [[ -f ${TC_DIR}/${tc}.out ]]; then
		...
		if [[ -f ${TC_DIR}/${tc}.in ]]; then
			./${SDRIVER} -t ${TC_DIR}/${tc}.in -s ./${ORIG}.${SHELL_ARCH} > ${OUTPUT}/${tc}.orig 2>&1;
#                        cp ${OUTPUT}/${tc}.orig ${TC_DIR}/${tc}.out;
		else
			${TC_DIR}/${ORIG}.${SHELL_ARCH} ${ARGS} > ${OUTPUT}/${tc}.orig 2>&1;		
		fi
	fi
