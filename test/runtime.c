...
{ 
    if (strcmp(cmd->argv[0], "jobs") == 0) {
      ...
      if (cmd->argc > 1){
        int returnValue1;
        returnValue1 = chdir(cmd->argv[1]);
        if (returnValue1 == -1){
          printf("chdir call failed");
          exit(0);
        }
      }
      else{
        int returnValue2;
        returnValue2 = chdir(getenv("HOME"));
        if (returnValue2 == -1){
          printf("chdir call failed");
          exit(0);
        }
...
void RunCmdRedirInOut(commandT* cmd, char* inFile, char* outFile)
{
  ...
  //open file for in and out source
  //0644 is the opinion for the operation
  int fOutId = open(outFile, O_WRONLY | O_CREAT, 0644);
  int fInId = open(inFile, O_RDONLY);
  //1 is standard output, 0 is standard input
  //backup the standard output and input
  ...
  /* This is run by the child. execute the command */
    ...
    /* This is run by the parent. */
      ...
      if(cmd->bg){
        ...
        fgpid = pid;
        // the fg job also add to the list, with the "isbg" parameter set to FALSE
